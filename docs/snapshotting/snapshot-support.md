...
- The _memory file_ and _microVM state file_ are generated by Firecracker on
  snapshot creation. The files are `fsync`'d by default (see
  [Syncing snapshot files](#syncing-snapshot-files)); the block device backing
  files are always `fsync`'d to disk.
